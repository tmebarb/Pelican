    ...
    <div class="container">
       <div class="section">
          <div class="row">
            ...
            <div class="input-field col s3">
                <button class="btn waves-effect waves-light red darken-4" type="button" name="action" id="mondayadd">Add</button>
            </div>
            <?php } else { 
                $first = true;
                foreach ($mondayStartSlots as $index => $startTime) {
                 ...
                 <?php if (!$first): ?>
                    ...
                    <div class="input-field col s3">
                        <input id="<?php echo 'MondayStartWith'.$index ?>" type="text" class="time input-field ui-timepicker-input" name="mondaystart[]" autocomplete="off" value="<?php echo $startTime ?>">
                        <label for="<?php echo 'MondayStartWith'.$index ?>">Start Time</label>
                    </div>
                    <div class="input-field col s3">
                        <input id="<?php echo 'MondayEndWith'.$index ?>" type="text" class="time input-field ui-timepicker-input" name="mondayend[]" autocomplete="off" value="<?php echo $mondayEndSlots[$index] ?>">
                        <label for="<?php echo 'MondayEndWith'.$index ?>">End Time</label>
                    </div>

                    <script>
                        $(document).ready(function(){
                            $("<?php echo '#MondayStartWith'.$index ?>").timepicker({  });
                            $("<?php echo '#MondayEndWith'.$index ?>").timepicker({  });
                            $("<?php echo '#MondayEndWith'.$index ?>").focusout(function() {
                            if (!$("<?php echo '#MondayEndWith'.$index ?>").val()) {
                                    alert("Enter Start time first")
                                    $("<?php echo '#MondayEndWith'.$index ?>").val("")
                                } else if ($("<?php echo '#MondayStartWith'.$index ?>").val() >= $("<?php echo '#MondayEndWith'.$index ?>").val()) {
                                    alert("Start time should before than end time")
                                    $("<?php echo '#MondayEndWith'.$index ?>").val("")
                                }
                            });
                            $("<?php echo '#MondayStartWith'.$index ?>").focusout(function() {
                                if ($("<?php echo '#MondayEndWith'.$index ?>").val()) {
                                    if ($("<?php echo '#MondayStartWith'.$index ?>").val() >= $("<?php echo '#MondayEndWith'.$index ?>").val()) {
                                        alert("Start time should before than end time")
                                        $("<?php echo '#MondayStartWith'.$index ?>").val("")
                                    }
                                }
                            });    
                        });
